...
#| fig-cap: "Reliability diagram of the calibrated dataset using the default settings of [ReliabilityDiagrams.jl]{.pkg} and the plotting packages."
#| fig-subcap:
#|   - "[Makie.jl]{.pkg}"
#|   - "[Plots.jl]{.pkg}"
#| layout-ncol: 2
...
If the computational cost is found to be prohibitive, the choice $m(n) = [\sqrt{n}]$ where $[\cdot]$ denotes the closest integer [@Zaremba:2013] is a possible trade-off.
...
For the number of samples ($n = 1000$) and the kernel chosen here, the more powerful yet computationally more expensive asymptotic test can be performed fast (see computational details below):
...
One caveat though is the very small number of low-entropy predictions of `LogReg` and `NaiveBayes` which decreases the power of the calibration tests and might even make the asymptotic approximation invalid.
