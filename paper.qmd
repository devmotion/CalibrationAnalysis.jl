...
format:
    jss-latex:
        ...
        header-includes: |
          ...
          \usepackage{tikz}
          \usetikzlibrary{matrix,fit,backgrounds,decorations.pathreplacing}
...
Probabilistic predictive models are used prominently in meteorology [see, e.g., @Cooke1906A; @Cooke1906B; @Murphy:1977; @DeGroot:1983; @ivarssonProbabilityForecastingSweden1986; @Broecker:2007] but also arise more generally in machine learning models and as posterior predictive distributions in Bayesian inference.
...
[CalibrationAnalysis.jl]{.pkg} supports standard reliability diagrams as well as non-standard modifications thereof, such as consistency bars [@Broecker:2007], that simplify their statistical interpretation.

A key feature of [CalibrationAnalysis.jl]{.pkg} are calibration error estimators for general probabilistic models.
Most existing research and tools focus on calibration analysis of classification models, with the notable exception of a [Python]{.proglang} package for distribution calibration of regression models [@Song:2019].
In contrast, [CalibrationAnalysis.jl]{.pkg} supports the estimation of calibration errors for both classification and regression models.
...
The [Python]{.proglang} package [t-cal]{.pkg} implements another calibration test based on the ECE [@leeTCalOptimalTest2022].
...
For instance, for a feature $X$ the model *predicts a distribution of $Y$* which we denote with $P_X$ to illustrate its dependence on $X$.
...
Such probabilistic forecasts allow to quantify uncertainties and are therefore used for many prediction problems.
...
For instance, the ideal model $\Prob(Y \,|\, X)$ is calibrated but generally there are multiple, also less accurate, models:
...
Most commonly, calibration is defined and analyzed for binary classification problems where the prediction $P_X$ is a Bernoulli distribution and typically identified with its success probability.
However, the definition in @eq-calibration has a wider applicability and covers multi-class classification as well as regression and other types of problems.
...
of $n$ probabilistic predictions $P_{X_1}, \ldots, P_{X_n}$ and corresponding observations $Y_1, \ldots, Y_n$.
...
For each prediction $p_{i}$, we compute an underconfident prediction $q_{i}$ as a convex combination of $p_{i}$ and the uniform categorical distribution $u$ with equal probability $1/d$ for each class.
...
That is, instead of the three-class classification model $P$ we study a binary classification model with extended features ($X$ and the mode of $P_X$) and binary targets (1 if $Y$ is equal to the mode of $P_X$, and 0 otherwise).
The predictions of this binary model are Bernoulli distributions whose success probabilities are the confidence estimates for the mode of $P_X$, i.e., $\max_y P_X(\{y\})$.
...
#| fig-cap: "Reliability diagrams of the calibrated dataset using the default settings of [ReliabilityDiagrams.jl]{.pkg} and the plotting packages."
#| fig-subcap:
#|   - "[Makie.jl]{.pkg}"
#|   - "[Plots.jl]{.pkg}"
#| layout-ncol: 2
...
The resulting Figures [-@fig-examples-reldiag-1] and [-@fig-examples-reldiag-2] show that the default reliability diagrams generated by [ReliabilityDiagrams.jl]{.pkg} differ from examples found in the literature [see, e.g., the discussion in @Broecker:2007]:
 ...
 1. By default, the average deviation of the empirical frequencies from the confidence estimates is plotted instead of the average empirical frequencies as in classic reliability diagrams [@Vaicenavicius:2019].
 ...
 2. By default, consistency bars are added that cover the range of the deviations that are likely to be observed with 95% probability *if* the model is calibrated [@Broecker:2007].
...
The motivation for consistency bars is that reliability diagrams can be difficult to interpret and sometimes even be misleading due to the randomness of the validation data (see, e.g., the example by @Broecker:2007).
...
let ps_binary = ps_binary, qs_binary = qs_binary, ys_binary = ys_binary
    ...
    reliability!(ax3, ps_binary, ys_binary; xlabel="", ylabel="", label="data")

    ax4 = Axis(fig[2, 2]; yticksvisible=false, yticklabelsvisible=false)
    hlines!(ax4, 0; color=:black, linestyle=:dash, label="ideal")
    reliability!(ax4, qs_binary, ys_binary; xlabel="", ylabel="", label="data")
...
![Illustration of the terms in the unbiased (left) and biased (right) estimators of the squared KCE in @eq-skce-estimators, highlighted in gray.](diagram.tex){#fig-kce-blocks}
...
If $n$ is not very large, we thus recommend $m = n$ as a default choice.
If the computational cost is found to be prohibitive, the choice $m(n) = [\sqrt{n}]$ where $[\cdot]$ denotes the closest integer is a possible trade-off [@Zaremba:2013].
...
We create the minimum-variance unbiased estimator of the squared KCE with this `kernel`:
...
The estimator `skce` can then be used to estimate the squared KCE from a validation dataset.
...
#| fig-cap: "Estimates of the squared KCE for different types of kernel $k_{P}$ on the space of predictions and varying length scales."
#| fig-format: pdf
let ps = ps, qs = qs, ys = ys
    # Compute SKCE estimates for different kernels and varying length scales
    lengthscales = exp10.(-3:1)
    kernels = (
        GaussianKernel(), LaplacianKernel(), RationalKernel(), RationalQuadraticKernel()
    )
    names = ["Gaussian", "Laplacian", "rational", "rational quadratic"]
    n = length(lengthscales) * length(kernels) # number of configurations for each model
    estimates = Float64[]
    sizehint!(estimates, 2 * n)
    for predictions in (ps, qs), k in kernels, lengthscale in lengthscales
        push!(
            estimates,
    ...
    data = DataFrame(;
        calibrated=vcat(fill(true, n), fill(false, n)),
        lengthscale=categorical(repeat(lengthscales, 2 * length(kernels))),
        kernel=categorical(repeat(names; inner=length(lengthscales), outer=2)),
        skce=estimates,
    )
...
However, if the length scale is too small (0.001) or too large (10.0), there is no clear difference between the estimates of the two models.
...
Unfortunately, the estimation of the ECE from a finite data set of predictions and observed outcomes is challenging as, in contrast to the estimators of the squared KCE, typically it requires to estimate the conditional distribution $\Prob(Y \,|\, P_X)$.
This is difficult in particular if $Y$ is a non-binary, real-valued, or possibly even higher-dimensional random variable.
...
let ps = ps, qs = qs, ys = ys
    ...
    n = length(nbins) * length(distances) # number of configurations for each model
    estimates = Float64[]
    sizehint!(estimates, 2 * n)
    for predictions in (ps, qs), d in distances, b in nbins
        push!(estimates, ECE(UniformBinning(b), d)(ColVecs(predictions), ys))
    end

    # Collect results in a data frame
    data = DataFrame(;
        calibrated=vcat(fill(true, n), fill(false, n)),
        nbins=repeat(nbins, 2 * length(distances)),
        distance=categorical(repeat(names; inner=length(nbins), outer=2)),
        ece=estimates,
    )
...
Generally, the estimates of the uncalibrated model seem to be larger than the ones for the calibrated model which suggests that it indeed is "less calibrated" than the calibrated one.
...
For the number of samples ($n = 1000$) and the kernel chosen here, the more powerful yet computationally more expensive asymptotic test can be performed sufficiently fast (see computational details below):
...
#| fig-format: pdf
let ps = ps, qs = qs, ys = ys
    # Compute p value estimates for different kernels and varying length scales
    lengthscales = exp10.(-3:1)
    kernels = (
        GaussianKernel(), LaplacianKernel(), RationalKernel(), RationalQuadraticKernel()
    )
...
It is also interesting that the $p$ value estimates of the uncalibrated model for a length scale of 0.01 are consistently smaller than the ones for a length scale of 10 *even though* one might expect the exactly opposite result from the raw estimates of the squared KCE.
...
In this section we demonstrate how [CalibrationAnalysis.jl]{.pkg} can be applied to a simple classification problem with real data.
...
@fig-penguins shows the distribution of the flipper length and body mass in the training and validation dataset.

```{julia}
#| echo: false
#| label: fig-penguins
#| fig-cap: "Measurements of flipper length and body mass in the Palmer penguin dataset."
#| fig-format: pdf
...
We want to emphasize that we do not intend to make any general comments about the calibration of these model classes but only want to illustrate how [CalibrationAnalysis.jl]{.pkg} can be used for comparing actual machine learning models.
...
The error rate does not convey any information about the confidence estimates for each mode.
...
#| fig-cap: "Estimates of the squared kernel calibration error of the three classification models. The left figure shows estimates for all predictions on the validation dataset whereas the estimates in the right figure only consider low-entropy predictions on the validation dataset."
#| fig-format: pdf
...
Hence the calibration error estimates confirm the impression we gained from the reliability diagrams of the confidence estimates in @fig-penguins-reldiags:
...
However, we do not know if `LogReg` and `NaiveBayes` are actually calibrated and `XGBoost` uncalibrated, or if the differences can be explained, e.g., by the different distributions of predictions.
...
However, in @fig-penguins-skce-estimates the errors seem small also for low-entropy predictions, in particular compared with the estimates for `XGBoost`, and arguably it is impossible to notice that the calibration errors are unexpectedly large for a calibrated model solely on @fig-penguins-skce-estimates.
One caveat though is the very small number of low-entropy predictions of `LogReg` and `NaiveBayes` which might make the asymptotic approximation invalid and decreases the power of the calibration tests.
...
We applied the software framework both on synthetic examples with datasets from a calibrated and an uncalibrated model and for analyzing different models for a classification problem with real data.
